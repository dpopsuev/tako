...
func NewFridge() Scenario {
	...
	adv.AddInstrument("check_hunger", "Check if you are still hungry", capability.ReadAction, func(s map[string]any, _ string) string {
		if s["hungry"] == true {
	...
	adv.AddInstrument("look_fridge", "See what food is in the fridge", capability.ReadAction, func(s map[string]any, _ string) string {
	...
	adv.AddInstrument("take", "Take an item from the fridge. Input: item name (e.g. 'eggs')", capability.WriteAction, func(s map[string]any, input string) string {
	...
	adv.AddInstrument("cook", "Cook what you're holding. Requires stove to be on.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
	...
	adv.AddInstrument("eat", "Eat what's on the plate", capability.WriteAction, func(s map[string]any, _ string) string {
...
func NewDirtyRoom() Scenario {
	...
	adv.AddInstrument("look", "Look around the room to see what needs cleaning", capability.ReadAction, func(s map[string]any, _ string) string {
	...
	adv.AddInstrument("get_broom", "Get the broom from the closet", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["broom"] == "hand" {
	...
	adv.AddInstrument("sweep", "Sweep the floor. Requires broom in hand.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["broom"] != "hand" {
	...
	adv.AddInstrument("wash_dishes", "Wash the dishes on the table", capability.WriteAction, func(s map[string]any, _ string) string {
		s["table"] = []string{}
		return "dishes washed and put away"
	})

	adv.AddInstrument("take_out_trash", "Take the trash out", capability.WriteAction, func(s map[string]any, _ string) string {
		s["trash"] = "empty"
		return "trash taken out"
	})

	adv.AddInstrument("check_done", "Check if the room is fully clean", capability.ReadAction, func(s map[string]any, _ string) string {
		floor, _ := s["floor"].([]string)
		table, _ := s["table"].([]string)
		trash, _ := s["trash"].(string)
