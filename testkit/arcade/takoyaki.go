...
func NewTakoyaki(ctx context.Context, sensory cerebrum.Bus) Scenario {
	...
	adv.AddInstrument("turn_on_stove", "Turn on the stove and grill", capability.WriteAction, func(s map[string]any, _ string) string {
		s["stove"] = "on"
		return "stove and grill are on, ready to cook"
	})

	adv.AddInstrument("extinguish", "Put out a kitchen fire", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["fire"] != true {
	...
	adv.AddInstrument("prep_batter", "Mix takoyaki batter (flour, eggs, dashi)", capability.WriteAction, func(s map[string]any, _ string) string {
		s["batter"] = 6
		return "batter mixed, enough for 6 takoyaki balls"
	})

	adv.AddInstrument("prep_filling", "Chop octopus, green onion, pickled ginger for filling", capability.WriteAction, func(s map[string]any, _ string) string {
		s["cutting_board"] = "chopped filling"
		s["filling_ready"] = true
		return "filling chopped and ready"
	})

	adv.AddInstrument("cook_takoyaki", "Pour batter into takoyaki grill with filling. Takes a moment to cook.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
	...
	adv.AddInstrument("cook_tempura", "Fry tempura vegetables in the deep fryer. Takes a moment.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
			return "stove is off"
		}
		if s["fire"] == true {
	...
	adv.AddInstrument("cook_rice", "Start cooking rice on the stove. Takes a moment.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
			return "stove is off"
		}
		if s["fire"] == true {
	...
	adv.AddInstrument("serve", "Serve a completed dish to fill a pending order. Input: dish name (takoyaki, tempura, rice_bowl)", capability.WriteAction, func(s map[string]any, input string) string {
		if s["fire"] == true {
	...
	adv.AddInstrument("check_orders", "Check the current order queue", capability.ReadAction, func(s map[string]any, _ string) string {
		pending, _ := s["orders_pending"].([]string)
		served, _ := s["orders_served"].(int)
