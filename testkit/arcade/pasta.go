...
func NewPastaBolognese(ctx context.Context, sensory cerebrum.Bus) Scenario {
	...
	adv.AddInstrument("look_pantry", "Check what ingredients are in the pantry", capability.ReadAction, func(s map[string]any, _ string) string {
	...
	adv.AddInstrument("look_fridge", "Check what ingredients are in the fridge", capability.ReadAction, func(s map[string]any, _ string) string {
	...
	adv.AddInstrument("look_kitchen", "Check the current state of everything in the kitchen", capability.ReadAction, func(s map[string]any, _ string) string {
	...
	adv.AddInstrument("turn_on_stove", "Turn on the stove", capability.WriteAction, func(s map[string]any, _ string) string {
		s["stove"] = "on"
		return "stove is on"
	})

	adv.AddInstrument("chop_soffritto", "Chop onion, garlic, and carrot for the soffritto base", capability.WriteAction, func(s map[string]any, _ string) string {
		s["cutting_board"] = "chopped soffritto"
		s["soffritto"] = true
		return "onion, garlic, and carrot chopped for soffritto"
	})

	adv.AddInstrument("saute_soffritto", "Saute the chopped soffritto in the pan with olive oil. Takes a moment to soften.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
	...
	adv.AddInstrument("brown_meat", "Add ground beef to the pan and brown it. Takes a moment.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
			return "stove is off"
		}
		pan, _ := s["pan"].(string)
	...
	adv.AddInstrument("add_tomatoes", "Add canned tomatoes to the pan to start the sauce. It needs to simmer.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["meat"] != "browned" {
	...
	adv.AddInstrument("boil_water", "Fill pot with water and put on stove. Takes a moment to boil.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["stove"] != "on" {
			return "stove is off, turn it on first"
		}
		s["pot"] = "heating"
	...
	adv.AddInstrument("cook_pasta", "Add pasta to boiling water. Takes a moment to cook al dente.", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["water"] != "boiling" {
	...
	adv.AddInstrument("drain_pasta", "Drain the cooked pasta", capability.WriteAction, func(s map[string]any, _ string) string {
	...
	adv.AddInstrument("grate_parmesan", "Grate fresh parmesan cheese", capability.WriteAction, func(s map[string]any, _ string) string {
		s["parmesan"] = "grated"
		return "parmesan freshly grated"
	})

	adv.AddInstrument("combine", "Toss drained pasta with the sauce in the pan", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["pasta"] != "drained" {
	...
	adv.AddInstrument("plate", "Plate the pasta bolognese with parmesan on top", capability.WriteAction, func(s map[string]any, _ string) string {
		pan, _ := s["pan"].(string)
	...
	adv.AddInstrument("eat", "Eat the plated dish", capability.WriteAction, func(s map[string]any, _ string) string {
		if s["plated"] != true {
	...
	adv.AddInstrument("check_served", "Check if the meal has been served and eaten", capability.ReadAction, func(s map[string]any, _ string) string {
		if s["eaten"] == true {
