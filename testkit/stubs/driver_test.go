...
func TestStubDriver_Handles_ReturnsConfiguredKind(t *testing.T) {
	d := NewStubDriver(SourceKindRepo)
	if got := d.Handles(); got != SourceKindRepo {
		t.Errorf("Handles() = %v, want %v", got, SourceKindRepo)
	}
}

func TestStubDriver_Handles_ReturnsDocKind(t *testing.T) {
	d := NewStubDriver(SourceKindDoc)
	if got := d.Handles(); got != SourceKindDoc {
		t.Errorf("Handles() = %v, want %v", got, SourceKindDoc)
	}
}

func TestStubDriver_Ensure_TracksCall(t *testing.T) {
	d := NewStubDriver(SourceKindRepo)
	src := &Source{Name: "test-repo"}
...
func TestStubDriver_Read_ReturnsCannedData(t *testing.T) {
	...
	data, err := d.Read(context.Background(), &Source{Name: "myrepo"}, "main.go")
	if err != nil {
		t.Fatalf("Read: %v", err)
	}
...
func TestStubDriver_Search_ReturnsCannedResults(t *testing.T) {
	d := NewStubDriver(SourceKindRepo)
	want := []SearchResult{{Path: "foo.go", Line: 10, Snippet: "func Foo()"}}
	d.WithSearchData("myrepo:Foo", want)

	got, err := d.Search(context.Background(), &Source{Name: "myrepo"}, "Foo", 10)
	if err != nil {
...
func TestStubDriver_List_ReturnsCannedEntries(t *testing.T) {
	d := NewStubDriver(SourceKindRepo)
	want := []ContentEntry{{Path: "src/", IsDir: true}}
	d.WithListData("myrepo:.", want)

	got, err := d.List(context.Background(), &Source{Name: "myrepo"}, ".", 1)
	if err != nil {
		t.Fatalf("List: %v", err)
	}
...
func TestStubDriver_SetError_InjectsError(t *testing.T) {
	...
	_, err := d.Read(context.Background(), &Source{Name: "x"}, "y")
	if !errors.Is(err, injected) {
		t.Errorf("Read error = %v, want %v", err, injected)
	}

	err = d.Ensure(context.Background(), &Source{Name: "x"})
	if !errors.Is(err, injected) {
...
func TestStubDriver_Reset_ClearsState(t *testing.T) {
	...
	_ = d.Ensure(context.Background(), &Source{Name: "x"})
	...
	if err := d.Ensure(context.Background(), &Source{Name: "x"}); err != nil {
...
func TestStubDriver_ThreadSafety(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			_, _ = d.Read(context.Background(), &Source{Name: "repo"}, "file.go")
			_ = d.Ensure(context.Background(), &Source{Name: "repo"})
		}()
	}
	wg.Wait()
