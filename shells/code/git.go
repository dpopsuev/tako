...
func (f *gitStatusFunc) Execute(ctx context.Context, input json.RawMessage) (shell.Result, error) {
	out, err := gitExec(ctx, f.root, "status", "--porcelain", "-b")
	if err != nil {
		return shell.ErrorResult(fmt.Sprintf("git_status: %s", err)), nil
	}
	return shell.TextResult(out), nil
}
...
func (f *gitDiffFunc) Execute(ctx context.Context, input json.RawMessage) (shell.Result, error) {
	var in struct {
	...
	if err != nil {
		return shell.ErrorResult(fmt.Sprintf("git_diff: %s", err)), nil
	}
	if out == "" {
		return shell.TextResult("no changes"), nil
	}
	return shell.TextResult(out), nil
}
...
func (f *gitCommitFunc) Execute(ctx context.Context, input json.RawMessage) (shell.Result, error) {
	var in struct {
		Message string   `json:"message"`
		Files   []string `json:"files"`
	}
	if err := json.Unmarshal(input, &in); err != nil {
		return shell.Result{}, fmt.Errorf("git_commit: %w", err)
	}
	if in.Message == "" {
		return shell.ErrorResult("git_commit: message required"), nil
	}

	if len(in.Files) > 0 {
		args := append([]string{"add"}, in.Files...)
		if _, err := gitExec(ctx, f.root, args...); err != nil {
			return shell.ErrorResult(fmt.Sprintf("git_commit: stage failed: %s", err)), nil
		}
	}

	out, err := gitExec(ctx, f.root, "commit", "-m", in.Message)
	if err != nil {
		return shell.ErrorResult(fmt.Sprintf("git_commit: %s\n%s", err, out)), nil
	}

	hash, _ := gitExec(ctx, f.root, "rev-parse", "--short", "HEAD")
	return shell.TextResult(fmt.Sprintf("committed %s: %s", strings.TrimSpace(hash), in.Message)), nil
}
