...
func TestExperiment_FridgeTemperatureCurve(t *testing.T) {
	...
	for i := 0; i < sessions; i++ {
		...
		if err := agent.Run(ctx, scenario.Need); err != nil {
			cancel()
		...
		t.Logf("session %d: turns=%d sealed=%v distance=%.2f pressure=%.2f solved=%v pipes=%d",
			i+1, sr.Turns, sr.Sealed, sr.Distance, sr.Pressure, sr.Solved, reflexStore.Len())
	}
