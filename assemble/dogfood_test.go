	...
	completer := &scriptedCompleter{
		turns: []tangle.Completion{
			...
			},
			{
				...
				ToolCalls: []tangle.ToolCall{
					{ID: "c2", Name: "read_file", Input: json.RawMessage(`{"path":"calc.go"}`)},
				},
			},
			{
		...
		turns: []tangle.Completion{
			{
				...
				ToolCalls: []tangle.ToolCall{
					{ID: "c1", Name: "read_file", Input: json.RawMessage(`{"path":"nonexistent.go"}`)},
				},
			},
			{
				Content: "File doesn't exist. Reading main.go instead.",
				ToolCalls: []tangle.ToolCall{
					{ID: "c2", Name: "read_file", Input: json.RawMessage(`{"path":"main.go"}`)},
				},
			},
			{
		...
		turns: []tangle.Completion{
			{
				...
				ToolCalls: []tangle.ToolCall{
					{ID: "c1", Name: "spawn_agent", Input: json.RawMessage(`{"task":"find all Go files","type":"explore","max_turns":3}`)},
				},
			},
			{
