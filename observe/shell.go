...
func WrapCapability(cap shell.Capability, pool ergograph.Ledger, tracer trace.Tracer) shell.Capability {
	if cap.Execute == nil {
		return cap
	}
	inner := cap.Execute
	cap.Execute = func(ctx context.Context, input json.RawMessage) (shell.Result, error) {
