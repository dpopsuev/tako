...
// Category: DSL & Build — aliases to circuit/ package + build-time graph construction.
...
func resolveNode(def *CircuitDef, nd NodeDef, reg GraphRegistries) (Node, error) {
...
func resolveHandler(def *CircuitDef, nd NodeDef, reg GraphRegistries, elem Element) (Node, error) {
	handler := nd.Handler
	if handler == "" {
		handler = nd.Name
	}
